...
public class GoldForecastPromptBuilder {
    ...
    public static final String PROMPT_VERSION = "gold-direction-forecast-prompt-v1";
    ...
    public GoldForecastPrompt build(StoredGoldResearchSnapshot record) {
        ...
        String content = """
                你是个人黄金投资研究助手。请基于给定正式快照，预测黄金下一有效交易日的方向。
                ...
                【方向合同】
                后续真实涨跌幅大于 0.5%% 为 BULLISH；小于 -0.5%% 为 BEARISH；其余包括两个边界值均为 NEUTRAL（中性）。
                ...
                """.formatted(
                record.id(), snapshot.analysisDate(), snapshot.latestGoldDate(),
                snapshot.latestRealRateDate(), snapshot.latestDollarIndexDate(),
                snapshot.researchVersion(), snapshot.gold().currentPrice().toPlainString(),
